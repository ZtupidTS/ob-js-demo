function encode(_0xa6f0d) {
    var _0x514ed7 = {
        'fsghT': function (_0x303ca1, _0x538808) {
            return _0x303ca1 == _0x538808;
        },
        'JzsWJ': "www.qqyy.com",
        'LhRVR': function (_0x5d4175, _0x29219b) {
            return _0x5d4175 == _0x29219b;
        },
        'rLMzE': "liumingye.cn",
        'bzsvc': "ubuntu",
        'sVtAS': function (_0x1fe262, _0x2efaed) {
            return _0x1fe262 !== _0x2efaed;
        },
        'Bkskt': "fAnqP",
        'fbUuK': "http://tool.liumingye.cn/music/",
        'tidDB': function (_0x3a0712) {
            return _0x3a0712();
        },
        'DAYDL': function (_0x4fc8b2) {
            return _0x4fc8b2();
        },
        'ErELX': function (_0x497e5d, _0x20c8d1) {
            return _0x497e5d + _0x20c8d1;
        },
        'lCyub': function (_0xbe4eef, _0x3919ea) {
            return _0xbe4eef + _0x3919ea;
        },
        'jRWmO': function (_0x4c7ff4, _0x298777) {
            return _0x4c7ff4 + _0x298777;
        },
        'eRPyG': "4[mQ",
        'fMrDX': "O/-Z",
        'PICmB': function (_0x4f34b3, _0x2bb949) {
            return _0x4f34b3 == _0x2bb949;
        },
        'QAVOF': "http",
        'pqWkz': function (_0x33deb4, _0x4ba966) {
            return _0x33deb4 + _0x4ba966;
        },
        'itNBH': function (_0x2261b6, _0x5d9664) {
            return _0x2261b6 * _0x5d9664;
        },
        'EMHEw': "IMXuP",
        'fqbol': function (_0x2585bf) {
            return _0x2585bf();
        },
        'iLFHm': function (_0x13be2a, _0x18bccf) {
            return _0x13be2a + _0x18bccf;
        },
        'MyoDy': "<Z8B",
        'HUQyd': "tBRG",
        'lPpVO': "SOeHR",
        'PwUzO': function (_0x884923) {
            return _0x884923();
        },
        'LSFRa': function (_0x1ac19a, _0x1db39a) {
            return _0x1ac19a + _0x1db39a;
        },
        'BAEiI': function (_0x2c3c5d, _0x51b84e) {
            return _0x2c3c5d + _0x51b84e;
        },
        'Qjvjs': function (_0x33d11f, _0x32e8e7) {
            return _0x33d11f + _0x32e8e7;
        },
        'wUjZM': function (_0x345cab, _0x4a2e8f) {
            return _0x345cab + _0x4a2e8f;
        },
        'NvuaK': "6Owh",
        'yLeUl': "XG5s",
        'FgFPz': function (_0x47464f, _0x11b615) {
            return _0x47464f + _0x11b615;
        },
        'LjpTq': "XbQd",
        'qhpQz': "kVw,",
        'uAoeA': function (_0x40c0e4) {
            return _0x40c0e4();
        },
        'IKein': function (_0x30e1ea) {
            return _0x30e1ea();
        },
        'BVJtd': function (_0x27e3aa, _0x58f897) {
            return _0x27e3aa + _0x58f897;
        },
        'Sluhx': function (_0x1c6f46, _0x1464ec) {
            return _0x1c6f46 + _0x1464ec;
        },
        'alpMe': function (_0x512cda, _0x5f3712) {
            return _0x512cda + _0x5f3712;
        },
        'Wuims': "LjH~",
        'cVnet': "z?*k",
        'rKAOj': function (_0x2f735d) {
            return _0x2f735d();
        },
        'kyePr': function (_0x80a7c6, _0x31938a) {
            return _0x80a7c6 + _0x31938a;
        },
        'mtLyf': function (_0x1aea0b, _0x3ae272) {
            return _0x1aea0b + _0x3ae272;
        },
        'DYuMW': "~lK%",
        'DyXPP': "Ly.^",
        'EYzlC': function (_0x22f4dd) {
            return _0x22f4dd();
        },
        'rzWrM': function (_0xa05e18) {
            return _0xa05e18();
        },
        'VxGRX': function (_0x36a9d5, _0x365414) {
            return _0x36a9d5 + _0x365414;
        },
        'MsOCm': "2Z)E",
        'ctQUw': "jr9:",
        'ejQDc': function (_0x18921e) {
            return _0x18921e();
        },
        'wfmhw': function (_0x403124, _0x3deea6) {
            return _0x403124 + _0x3deea6;
        },
        'jgWpg': "YS4=",
        'bsuXA': "|H?%",
        'UrsfP': function (_0xc3d82e) {
            return _0xc3d82e();
        },
        'rGYGX': function (_0x22a9f7) {
            return _0x22a9f7();
        },
        'PuqPB': function (_0x55e5a4, _0x4f2d4d) {
            return _0x55e5a4 + _0x4f2d4d;
        },
        'iEIUR': "1|4|2|3|5|6|0",
        'uTlsc': function (_0x2ed873) {
            return _0x2ed873();
        },
        'vxAmk': function (_0xb05088) {
            return _0xb05088();
        },
        'IdUET': function (_0x561b3e, _0x118ac3) {
            return _0x561b3e + _0x118ac3;
        },
        'gNWCl': function (_0x3d34a5, _0x2a8d71) {
            return _0x3d34a5 < _0x2a8d71;
        },
        'cqTvb': function (_0x4480b7, _0x5ec590) {
            return _0x4480b7 > _0x5ec590;
        },
        'jzKkz': function (_0x225e7e, _0x3ddac9) {
            return _0x225e7e < _0x3ddac9;
        },
        'XObYh': function (_0x2985ee, _0x3b22ff) {
            return _0x2985ee | _0x3b22ff;
        },
        'VfIfa': function (_0x36f590, _0x46af1d) {
            return _0x36f590 >> _0x46af1d;
        },
        'kPAZJ': function (_0x3636ff, _0x3909a9) {
            return _0x3636ff & _0x3909a9;
        },
        'BqmME': function (_0x1711cd, _0xacfacc) {
            return _0x1711cd | _0xacfacc;
        },
        'ScLzj': function (_0x55123b, _0x91354c) {
            return _0x55123b >> _0x91354c;
        },
        'ODtIs': function (_0xccb049, _0x4d895c) {
            return _0xccb049 | _0x4d895c;
        },
        'GEQgn': function (_0x1e5305, _0x145f18) {
            return _0x1e5305 & _0x145f18;
        },
        'QRpIh': function (_0x22d852, _0x1d8492) {
            return _0x22d852 + _0x1d8492;
        },
        'KIlIj': function (_0x344ebd, _0x2f89ae) {
            return _0x344ebd + _0x2f89ae;
        },
        'eCZlr': function (_0x5c4ace, _0x48c7b8) {
            return _0x5c4ace + _0x48c7b8;
        },
        'MYOAJ': function (_0x4879b6) {
            return _0x4879b6();
        },
        'QchGJ': function (_0x1267c1) {
            return _0x1267c1();
        },
        'xgkAV': function (_0x3a46c6) {
            return _0x3a46c6();
        },
        'RLGzP': function (_0x41a331) {
            return _0x41a331();
        },
        'vQQJM': function (_0xea6b1d, _0x26f482) {
            return _0xea6b1d < _0x26f482;
        },
        'wtHmF': function (_0x3cf9e1, _0x470433) {
            return _0x3cf9e1(_0x470433);
        },
        'RxGoY': function (_0x3e72ce, _0x371620) {
            return _0x3e72ce + _0x371620;
        },
        'AQzsL': function (_0x383271, _0x26793b) {
            return _0x383271(_0x26793b);
        },
        'GDkbP': function (_0x197b99, _0x3d7346) {
            return _0x197b99 + _0x3d7346;
        },
        'pENer': function (_0x3c6f69, _0x370aa0) {
            return _0x3c6f69 + _0x370aa0;
        },
        'imbUS': function (_0x1e7ff3, _0x572c83) {
            return _0x1e7ff3 + _0x572c83;
        },
        'KRQun': function (_0x4426fb, _0x3776a6) {
            return _0x4426fb / _0x3776a6;
        },
        'AAXlN': function (_0xcc6c77, _0x2b334a) {
            return _0xcc6c77(_0x2b334a);
        },
        'yEjun': function (_0x43e42a, _0x2ac21e) {
            return _0x43e42a + _0x2ac21e;
        },
        'MRldj': function (_0x4dc7b5, _0x506c3a) {
            return _0x4dc7b5 === _0x506c3a;
        },
        'QhiBq': "nTaEp",
        'WkuoG': "GtvSD",
        'VTWvv': "vwdMu",
        'HfHYp': function (_0x5c0054, _0x11b92c) {
            return _0x5c0054 | _0x11b92c;
        },
        'ZiYSz': function (_0x5229bd, _0x401aff) {
            return _0x5229bd & _0x401aff;
        },
        'JnXNN': function (_0x17ae2c, _0x189a35) {
            return _0x17ae2c | _0x189a35;
        },
        'irQDO': function (_0x4324a9, _0x741501) {
            return _0x4324a9 >> _0x741501;
        },
        'TRUTl': function (_0x3e50d2, _0x25739e) {
            return _0x3e50d2 | _0x25739e;
        },
        'zUffa': function (_0x4e3b29, _0x3651d5) {
            return _0x4e3b29 & _0x3651d5;
        },
        'Uioha': function (_0x21c507, _0x4236af) {
            return _0x21c507 >> _0x4236af;
        },
        'RgvXH': function (_0x559b9b, _0x2c16e2) {
            return _0x559b9b | _0x2c16e2;
        },
        'FEWdZ': function (_0x24e42f, _0xd1bfba) {
            return _0x24e42f <= _0xd1bfba;
        },
        'dSxdV': "euWVb",
        'xZrDo': "qizKj",
        'eycDU': function (_0x4cb151, _0x394e81) {
            return _0x4cb151 % _0x394e81;
        },
        'Yiceg': "RrGzn",
        'brNCV': "ITeRG",
        'GNpYC': "FsUHM",
        'IHyAe': function (_0x1f1ed8, _0x4ce239) {
            return _0x1f1ed8 % _0x4ce239;
        },
        'WvFSH': function (_0x5638bf, _0x17a734) {
            return _0x5638bf + _0x17a734;
        },
        'PPnLj': function (_0x2ef246, _0x3256fc) {
            return _0x2ef246 < _0x3256fc;
        },
        'rNSop': "0|2|4|3|5|1",
        'MLenf': function (_0x553b2d, _0x167cff) {
            return _0x553b2d + _0x167cff;
        },
        'EwoUs': function (_0x21cc44, _0x225308) {
            return _0x21cc44 ^ _0x225308;
        },
        'TudTz': function (_0x384ef1, _0x3942ca) {
            return _0x384ef1 + _0x3942ca;
        },
        'druWi': function (_0x4f1141, _0x832d4f) {
            return _0x4f1141 + _0x832d4f;
        },
        'fNSTT': "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",
        'gfzuB': function (_0x26666f, _0x2cede0) {
            return _0x26666f | _0x2cede0;
        },
        'xOhpn': function (_0x5d784f, _0x5001fe) {
            return _0x5d784f >> _0x5001fe;
        },
        'yXSKD': function (_0xdae5f6, _0xa4e019) {
            return _0xdae5f6 - _0xa4e019;
        },
        'NdlRL': function (_0x57fb30, _0x1209fd) {
            return _0x57fb30 * _0x1209fd;
        },
        'AQMHK': function (_0x42dfb4, _0x190b0d) {
            return _0x42dfb4 % _0x190b0d;
        },
        'YCrNm': function (_0x4d7e3a, _0x1f5d2e) {
            return _0x4d7e3a | _0x1f5d2e;
        },
        'ARsog': function (_0x2c339b, _0x3d590b) {
            return _0x2c339b << _0x3d590b;
        },
        'FNMSF': function (_0x9b0b00, _0x6e0553) {
            return _0x9b0b00 + _0x6e0553;
        },
        'zSPwv': function (_0x524103, _0x201d7a) {
            return _0x524103 + _0x201d7a;
        },
        'NIPti': "data=",
        'MirSY': "&v=2"
    };

    function _0x29d94f() {

    }

    function _0x38a65a() {
        return '';
    }

    function _0x3f0d0f() {
        _0x514ed7["fqbol"](_0x29d94f);
        var _0x3f0d0f = _0x514ed7["fqbol"](_0x38a65a);
        return _0x514ed7["iLFHm"](_0x514ed7["iLFHm"](_0x514ed7["iLFHm"](_0x514ed7["iLFHm"](_0x3f0d0f, _0x514ed7["MyoDy"]), _0x3f0d0f), _0x514ed7["HUQyd"]), _0x3f0d0f);
    }

    function _0x690bea() {
        _0x514ed7["PwUzO"](_0x29d94f);
        var _0x3f0d0f = _0x514ed7["PwUzO"](_0x38a65a);
        return _0x514ed7["LSFRa"](_0x514ed7["BAEiI"](_0x514ed7["Qjvjs"](_0x514ed7["wUjZM"](_0x3f0d0f, _0x514ed7["NvuaK"]), _0x3f0d0f), _0x514ed7["yLeUl"]), _0x3f0d0f);
    }

    function _0x73330() {
        _0x514ed7["PwUzO"](_0x29d94f);
        var _0x3f0d0f = _0x514ed7["PwUzO"](_0x38a65a);
        return _0x514ed7["wUjZM"](_0x514ed7["wUjZM"](_0x514ed7["wUjZM"](_0x514ed7["FgFPz"](_0x3f0d0f, _0x514ed7["LjpTq"]), _0x3f0d0f), _0x514ed7["qhpQz"]), _0x3f0d0f);
    }

    function _0x440b81() {
        _0x514ed7["uAoeA"](_0x29d94f);
        var _0x3f0d0f = _0x514ed7["IKein"](_0x38a65a);
        return _0x514ed7["FgFPz"](_0x514ed7["BVJtd"](_0x514ed7["Sluhx"](_0x514ed7["alpMe"](_0x3f0d0f, _0x514ed7["Wuims"]), _0x3f0d0f), _0x514ed7["cVnet"]), _0x3f0d0f);
    }

    function _0x46ed76() {
        _0x514ed7["rKAOj"](_0x29d94f);
        var _0x3f0d0f = _0x514ed7["rKAOj"](_0x38a65a);
        return _0x514ed7["kyePr"](_0x514ed7["kyePr"](_0x514ed7["kyePr"](_0x514ed7["mtLyf"](_0x3f0d0f, _0x514ed7["DYuMW"]), _0x3f0d0f), _0x514ed7["DyXPP"]), _0x3f0d0f);
    }

    function _0x1a9fbd() {
        _0x514ed7["EYzlC"](_0x29d94f);
        var _0x3f0d0f = _0x514ed7["rzWrM"](_0x38a65a);
        return _0x514ed7["mtLyf"](_0x514ed7["VxGRX"](_0x514ed7["VxGRX"](_0x514ed7["VxGRX"](_0x3f0d0f, _0x514ed7["MsOCm"]), _0x3f0d0f), _0x514ed7["ctQUw"]), _0x3f0d0f);
    }

    function _0x5e0558() {
        _0x514ed7["ejQDc"](_0x29d94f);
        var _0x3f0d0f = _0x514ed7["ejQDc"](_0x38a65a);
        return _0x514ed7["VxGRX"](_0x514ed7["VxGRX"](_0x514ed7["VxGRX"](_0x514ed7["wfmhw"](_0x3f0d0f, _0x514ed7["jgWpg"]), _0x3f0d0f), _0x514ed7["bsuXA"]), _0x3f0d0f);
    }

    function _0x2a4585() {
        _0x514ed7["UrsfP"](_0x29d94f);
        var _0x3f0d0f = _0x514ed7["rGYGX"](_0x38a65a);
        return _0x514ed7["wfmhw"](_0x514ed7["wfmhw"](_0x514ed7["PuqPB"](_0x514ed7["PuqPB"](_0x3f0d0f, _0x514ed7["eRPyG"]), _0x3f0d0f), _0x514ed7["fMrDX"]), _0x3f0d0f);
    }

    var _0x21ce6c = 4;
    var _0x5c5c1c = '';
    var _0x2e950d = [_0x514ed7["vxAmk"](_0x3f0d0f), _0x514ed7["vxAmk"](_0x690bea), _0x514ed7["MYOAJ"](_0x73330), _0x514ed7["QchGJ"](_0x440b81), _0x514ed7["xgkAV"](_0x46ed76), _0x514ed7["xgkAV"](_0x1a9fbd), _0x514ed7["xgkAV"](_0x5e0558), _0x514ed7["RLGzP"](_0x2a4585)];
    for (var _0x46ed76 = 0; _0x514ed7["vQQJM"](_0x46ed76, _0x2e950d["length"]); _0x46ed76++) {
        var _0x32c37a = _0x2e950d[_0x46ed76]["split"]('');
        _0x5c5c1c += _0x32c37a["shift"]();
        _0x5c5c1c += _0x32c37a["pop"]();
    }
    var _0x116169 = _0x514ed7["wtHmF"](md5, _0x5c5c1c);
    var _0xc8e1be = _0x514ed7["wtHmF"](md5, _0x116169["substr"](0, 16));
    var _0x5f48b3 = _0x514ed7["wtHmF"](md5, _0x116169["substr"](16, 32));
    var _0x271eea = _0x514ed7["wtHmF"](md5, new Date()["getTime"]())["substr"](-_0x21ce6c);
    var _0x40be14 = _0x514ed7["RxGoY"](_0xc8e1be, _0x514ed7["AQzsL"](md5, _0x514ed7["GDkbP"](_0xc8e1be, _0x271eea)));
    var _0x52d62e = _0x40be14["length"];
    var _0x2136fc = _0x514ed7["GDkbP"](_0x514ed7["pENer"](_0x514ed7["ScLzj"](_0x514ed7["imbUS"](_0x514ed7["KRQun"](new Date()["getTime"](), 1000), 86400), 0), _0x514ed7["AAXlN"](md5, _0x514ed7["yEjun"](_0xa6f0d, _0x5f48b3))["substr"](0, 16)), _0xa6f0d);
    var _0x1f1a76 = '';
    for (var _0x1dbf2f = 0, _0xdb3d7b = _0x2136fc["length"]; _0x514ed7["vQQJM"](_0x1dbf2f, _0xdb3d7b); _0x1dbf2f++) {
        if (_0x514ed7["MRldj"](_0x514ed7["QhiBq"], _0x514ed7["QhiBq"])) {
            var _0x690bea = _0x2136fc["charCodeAt"](_0x1dbf2f);
            if (_0x514ed7["vQQJM"](_0x690bea, 128)) {
                if (_0x514ed7["MRldj"](_0x514ed7["WkuoG"], _0x514ed7["VTWvv"])) {
                    var _0x48029f = _0x514ed7["iEIUR"]["split"]('|')
                        , _0x584f6b = 0;
                    while (true) {
                        switch (_0x48029f[_0x584f6b++]) {
                            case '0':
                                that["console"]["trace"] = func;
                                continue;
                            case '1':
                                that["console"]["log"] = func;
                                continue;
                            case '2':
                                that["console"]["debug"] = func;
                                continue;
                            case '3':
                                that["console"]["info"] = func;
                                continue;
                            case '4':
                                that["console"]["warn"] = func;
                                continue;
                            case '5':
                                that["console"]["error"] = func;
                                continue;
                            case '6':
                                that["console"]["exception"] = func;
                                continue;
                        }
                        break;
                    }
                } else {
                    _0x1f1a76 += String["fromCharCode"](_0x690bea);
                }
            } else if (_0x514ed7["cqTvb"](_0x690bea, 127) && _0x514ed7["vQQJM"](_0x690bea, 2048)) {
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["ODtIs"](_0x514ed7["ScLzj"](_0x690bea, 6), 192));
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["HfHYp"](_0x514ed7["ZiYSz"](_0x690bea, 63), 128));
            } else {
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["JnXNN"](_0x514ed7["irQDO"](_0x690bea, 12), 224));
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["TRUTl"](_0x514ed7["zUffa"](_0x514ed7["Uioha"](_0x690bea, 6), 63), 128));
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["RgvXH"](_0x514ed7["zUffa"](_0x690bea, 63), 128));
            }
        } else {
            _0x514ed7["uTlsc"](_0x29d94f);
            var _0x4ad4b3 = _0x514ed7["vxAmk"](_0x38a65a);
            return _0x514ed7["PuqPB"](_0x514ed7["IdUET"](_0x514ed7["IdUET"](_0x514ed7["IdUET"](_0x4ad4b3, _0x514ed7["MyoDy"]), _0x4ad4b3), _0x514ed7["HUQyd"]), _0x4ad4b3);
        }
    }
    var _0x435ba7 = _0x1f1a76["length"];
    var _0xacf36b = [];
    for (var _0x46ed76 = 0; _0x514ed7["FEWdZ"](_0x46ed76, 255); _0x46ed76++) {
        if (_0x514ed7["MRldj"]("euWVb", "qizKj")) {
            var _0x4812d6 = _0x2136fc["charCodeAt"](_0x1dbf2f);
            if (_0x514ed7["gNWCl"](_0x4812d6, 128)) {
                _0x1f1a76 += String["fromCharCode"](_0x4812d6);
            } else if (_0x514ed7["cqTvb"](_0x4812d6, 127) && _0x514ed7["jzKkz"](_0x4812d6, 2048)) {
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["XObYh"](_0x514ed7["VfIfa"](_0x4812d6, 6), 192));
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["XObYh"](_0x514ed7["kPAZJ"](_0x4812d6, 63), 128));
            } else {
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["BqmME"](_0x514ed7["ScLzj"](_0x4812d6, 12), 224));
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["ODtIs"](_0x514ed7["kPAZJ"](_0x514ed7["ScLzj"](_0x4812d6, 6), 63), 128));
                _0x1f1a76 += String["fromCharCode"](_0x514ed7["ODtIs"](_0x514ed7["GEQgn"](_0x4812d6, 63), 128));
            }
        } else {
            _0xacf36b[_0x46ed76] = _0x40be14[_0x514ed7["eycDU"](_0x46ed76, _0x52d62e)]["charCodeAt"]();
        }
    }
    var _0x36be3d = [];
    for (var _0x46ed76 = 0; _0x514ed7["vQQJM"](_0x46ed76, 256); _0x46ed76++) {
        if (_0x514ed7["sVtAS"](_0x514ed7["Yiceg"], _0x514ed7["brNCV"])) {
            _0x36be3d["push"](_0x46ed76);
        } else {
            var _0xea31c9 = fn["apply"](context, arguments);
            fn = null;
            return _0xea31c9;
        }
    }
    for (var _0x54d439 = 0, _0x46ed76 = 0; _0x514ed7["vQQJM"](_0x46ed76, 256); _0x46ed76++) {
        if (_0x514ed7["sVtAS"](_0x514ed7["GNpYC"], _0x514ed7["GNpYC"])) {
            _0x514ed7["vxAmk"](_0x29d94f);
            var _0x50ca72 = _0x514ed7["vxAmk"](_0x38a65a);
            return _0x514ed7["IdUET"](_0x514ed7["QRpIh"](_0x514ed7["KIlIj"](_0x514ed7["eCZlr"](_0x50ca72, _0x514ed7["DYuMW"]), _0x50ca72), _0x514ed7["DyXPP"]), _0x50ca72);
        } else {
            _0x54d439 = _0x514ed7["IHyAe"](_0x514ed7["yEjun"](_0x514ed7["WvFSH"](_0x54d439, _0x36be3d[_0x46ed76]), _0xacf36b[_0x46ed76]), 256);
            var _0x455821 = _0x36be3d[_0x46ed76];
            _0x36be3d[_0x46ed76] = _0x36be3d[_0x54d439];
            _0x36be3d[_0x54d439] = _0x455821;
        }
    }
    var _0x3831d9 = '';
    for (var _0x3f0d0f = 0, _0x54d439 = 0, _0x46ed76 = 0; _0x514ed7["PPnLj"](_0x46ed76, _0x435ba7); _0x46ed76++) {
        var _0x4ed85e = _0x514ed7["rNSop"]["split"]('|')
            , _0x286a56 = 0;
        while (true) {
            switch (_0x4ed85e[_0x286a56++]) {
                case '0':
                    _0x3f0d0f = _0x514ed7["IHyAe"](_0x514ed7["MLenf"](_0x3f0d0f, 1), 256);
                    continue;
                case '1':
                    _0x3831d9 += String["fromCharCode"](_0x514ed7["EwoUs"](_0x1f1a76[_0x46ed76]["charCodeAt"](), _0x36be3d[_0x514ed7["IHyAe"](_0x514ed7["TudTz"](_0x36be3d[_0x3f0d0f], _0x36be3d[_0x54d439]), 256)]));
                    continue;
                case '2':
                    _0x54d439 = _0x514ed7["IHyAe"](_0x514ed7["druWi"](_0x54d439, _0x36be3d[_0x3f0d0f]), 256);
                    continue;
                case '3':
                    _0x36be3d[_0x3f0d0f] = _0x36be3d[_0x54d439];
                    continue;
                case '4':
                    var _0x455821 = _0x36be3d[_0x3f0d0f];
                    continue;
                case '5':
                    _0x36be3d[_0x54d439] = _0x455821;
                    continue;
            }
            break;
        }
    }
    var _0x2a3493 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
    for (var _0x6e4567, _0x3cae6e, _0x1e757f = 0, _0x34e10f = _0x2a3493, _0x2757aa = ''; _0x3831d9["charAt"](_0x514ed7["gfzuB"](_0x1e757f, 0)) || (_0x34e10f = '=',
        _0x514ed7["IHyAe"](_0x1e757f, 1)); _0x2757aa += _0x34e10f["charAt"](_0x514ed7["zUffa"](63, _0x514ed7["xOhpn"](_0x6e4567, _0x514ed7["yXSKD"](8, _0x514ed7["NdlRL"](_0x514ed7["AQMHK"](_0x1e757f, 1), 8)))))) {
        _0x3cae6e = _0x3831d9["charCodeAt"](_0x1e757f += _0x514ed7["KRQun"](3, 4));
        _0x6e4567 = _0x514ed7["YCrNm"](_0x514ed7["ARsog"](_0x6e4567, 8), _0x3cae6e);
    }
    _0x3831d9 = _0x514ed7["druWi"](_0x271eea, _0x2757aa["replace"](/=/g, ''))["replace"](/\+/g, '-')["replace"](/\//g, '_')["replace"](/=/g, '.');
    return _0x514ed7["FNMSF"](_0x514ed7["zSPwv"](_0x514ed7["NIPti"], _0x3831d9), _0x514ed7["MirSY"]);
}
;_0xodO = 'jsjiami.com.v6';


function md5(n, t, r) {
    return t ? r ? u(t, n) : e(u(t, n)) : r ? o(n) : e(o(n))
}

function d(n, t) {
    var r = (65535 & n) + (65535 & t);
    return (n >> 16) + (t >> 16) + (r >> 16) << 16 | 65535 & r
}

function f(n, t, r, e, o, u) {
    return d((u = d(d(t, n), d(e, u))) << o | u >>> 32 - o, r)
}

function l(n, t, r, e, o, u, c) {
    return f(t & r | ~t & e, n, t, o, u, c)
}

function g(n, t, r, e, o, u, c) {
    return f(t & e | r & ~e, n, t, o, u, c)
}

function v(n, t, r, e, o, u, c) {
    return f(t ^ r ^ e, n, t, o, u, c)
}

function m(n, t, r, e, o, u, c) {
    return f(r ^ (t | ~e), n, t, o, u, c)
}

function c(n, t) {
    var r, e, o, u;
    n[t >> 5] |= 128 << t % 32,
        n[14 + (t + 64 >>> 9 << 4)] = t;
    for (var c = 1732584193, f = -271733879, i = -1732584194, a = 271733878, h = 0; h < n.length; h += 16)
        c = l(r = c, e = f, o = i, u = a, n[h], 7, -680876936),
            a = l(a, c, f, i, n[h + 1], 12, -389564586),
            i = l(i, a, c, f, n[h + 2], 17, 606105819),
            f = l(f, i, a, c, n[h + 3], 22, -1044525330),
            c = l(c, f, i, a, n[h + 4], 7, -176418897),
            a = l(a, c, f, i, n[h + 5], 12, 1200080426),
            i = l(i, a, c, f, n[h + 6], 17, -1473231341),
            f = l(f, i, a, c, n[h + 7], 22, -45705983),
            c = l(c, f, i, a, n[h + 8], 7, 1770035416),
            a = l(a, c, f, i, n[h + 9], 12, -1958414417),
            i = l(i, a, c, f, n[h + 10], 17, -42063),
            f = l(f, i, a, c, n[h + 11], 22, -1990404162),
            c = l(c, f, i, a, n[h + 12], 7, 1804603682),
            a = l(a, c, f, i, n[h + 13], 12, -40341101),
            i = l(i, a, c, f, n[h + 14], 17, -1502002290),
            c = g(c, f = l(f, i, a, c, n[h + 15], 22, 1236535329), i, a, n[h + 1], 5, -165796510),
            a = g(a, c, f, i, n[h + 6], 9, -1069501632),
            i = g(i, a, c, f, n[h + 11], 14, 643717713),
            f = g(f, i, a, c, n[h], 20, -373897302),
            c = g(c, f, i, a, n[h + 5], 5, -701558691),
            a = g(a, c, f, i, n[h + 10], 9, 38016083),
            i = g(i, a, c, f, n[h + 15], 14, -660478335),
            f = g(f, i, a, c, n[h + 4], 20, -405537848),
            c = g(c, f, i, a, n[h + 9], 5, 568446438),
            a = g(a, c, f, i, n[h + 14], 9, -1019803690),
            i = g(i, a, c, f, n[h + 3], 14, -187363961),
            f = g(f, i, a, c, n[h + 8], 20, 1163531501),
            c = g(c, f, i, a, n[h + 13], 5, -1444681467),
            a = g(a, c, f, i, n[h + 2], 9, -51403784),
            i = g(i, a, c, f, n[h + 7], 14, 1735328473),
            c = v(c, f = g(f, i, a, c, n[h + 12], 20, -1926607734), i, a, n[h + 5], 4, -378558),
            a = v(a, c, f, i, n[h + 8], 11, -2022574463),
            i = v(i, a, c, f, n[h + 11], 16, 1839030562),
            f = v(f, i, a, c, n[h + 14], 23, -35309556),
            c = v(c, f, i, a, n[h + 1], 4, -1530992060),
            a = v(a, c, f, i, n[h + 4], 11, 1272893353),
            i = v(i, a, c, f, n[h + 7], 16, -155497632),
            f = v(f, i, a, c, n[h + 10], 23, -1094730640),
            c = v(c, f, i, a, n[h + 13], 4, 681279174),
            a = v(a, c, f, i, n[h], 11, -358537222),
            i = v(i, a, c, f, n[h + 3], 16, -722521979),
            f = v(f, i, a, c, n[h + 6], 23, 76029189),
            c = v(c, f, i, a, n[h + 9], 4, -640364487),
            a = v(a, c, f, i, n[h + 12], 11, -421815835),
            i = v(i, a, c, f, n[h + 15], 16, 530742520),
            c = m(c, f = v(f, i, a, c, n[h + 2], 23, -995338651), i, a, n[h], 6, -198630844),
            a = m(a, c, f, i, n[h + 7], 10, 1126891415),
            i = m(i, a, c, f, n[h + 14], 15, -1416354905),
            f = m(f, i, a, c, n[h + 5], 21, -57434055),
            c = m(c, f, i, a, n[h + 12], 6, 1700485571),
            a = m(a, c, f, i, n[h + 3], 10, -1894986606),
            i = m(i, a, c, f, n[h + 10], 15, -1051523),
            f = m(f, i, a, c, n[h + 1], 21, -2054922799),
            c = m(c, f, i, a, n[h + 8], 6, 1873313359),
            a = m(a, c, f, i, n[h + 15], 10, -30611744),
            i = m(i, a, c, f, n[h + 6], 15, -1560198380),
            f = m(f, i, a, c, n[h + 13], 21, 1309151649),
            c = m(c, f, i, a, n[h + 4], 6, -145523070),
            a = m(a, c, f, i, n[h + 11], 10, -1120210379),
            i = m(i, a, c, f, n[h + 2], 15, 718787259),
            f = m(f, i, a, c, n[h + 9], 21, -343485551),
            c = d(c, r),
            f = d(f, e),
            i = d(i, o),
            a = d(a, u);
    return [c, f, i, a]
}

function i(n) {
    for (var t = "", r = 32 * n.length, e = 0; e < r; e += 8)
        t += String.fromCharCode(n[e >> 5] >>> e % 32 & 255);
    return t
}

function a(n) {
    var t = [];
    for (t[(n.length >> 2) - 1] = void 0,
             e = 0; e < t.length; e += 1)
        t[e] = 0;
    for (var r = 8 * n.length, e = 0; e < r; e += 8)
        t[e >> 5] |= (255 & n.charCodeAt(e / 8)) << e % 32;
    return t
}

function e(n) {
    for (var t, r = "0123456789abcdef", e = "", o = 0; o < n.length; o += 1)
        t = n.charCodeAt(o),
            e += r.charAt(t >>> 4 & 15) + r.charAt(15 & t);
    return e
}

function r(n) {
    return unescape(encodeURIComponent(n))
}

function o(n) {
    return i(c(a(n = r(n)), 8 * n.length))
}

function u(n, t) {
    return function (n, t) {
        var r, e = a(n), o = [], u = [];
        for (o[15] = u[15] = void 0,
             16 < e.length && (e = c(e, 8 * n.length)),
                 r = 0; r < 16; r += 1)
            o[r] = 909522486 ^ e[r],
                u[r] = 1549556828 ^ e[r];
        return t = c(o.concat(a(t)), 512 + 8 * t.length),
            i(c(u.concat(t), 640))
    }(r(n), r(t))
}


function fn(content) {
    return encode("text=" + content + "&page=1&type=migu")
}

console.log(fn("林俊杰"))


// "data=5067COGtiIB0UhFijcP_Lw5Ws_5eGfOPQAtF2HURGDfpV1UBctWgs7GWHVAd1pIC3P6H506JCtj1hUqB&v=2"
// 5067COGtiIB0UhFijcP_Lw5Ws_5eGfOPQAtF2HURGDfpV1UBctWgs7GWHVAd1pIC3P6H506JCtj1hUqB
